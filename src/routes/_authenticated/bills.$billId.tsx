...
function BillDetail() {
  ...
  const bill = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("bills")
        .select("*, lands(id, land_code, plot_number, location_description, landowners(full_name, phone, email))")
        .eq("id", billId)
        .single();
  ...
  const land = bill.data?.lands as unknown as
    | { id: string; land_code: string; plot_number: string | null; location_description: string | null; landowners: { full_name: string; phone: string | null; email: string | null } | null }
    | null;
    ...
    <AppShell
      ...
      actions={
        <Button asChild variant="outline" size="sm">
          <Link to="/bills"><ArrowLeft className="mr-1 h-4 w-4" /> Back</Link>
        </Button>
      }
    >
      <div className="grid gap-4 lg:grid-cols-3">
        <Card className="lg:col-span-2">
          ...
          <CardContent className="space-y-3 print:p-8">
            <div className="print-only rounded-md border border-border p-4">
              ...
              <div className="mt-3 grid grid-cols-2 gap-3 text-sm">
                <div>
                  ...
                  <p className="font-medium">{land?.land_code} · Plot {land?.plot_number ?? "—"}</p>
                  <p className="text-xs text-muted-foreground">{land?.location_description ?? ""}</p>
                </div>
                <div>
              ...
              <table className="mt-4 w-full text-sm">
                <tbody>
                  <tr className="border-t"><td className="py-2">Annual ground rent</td><td className="py-2 text-right">{formatCurrency(bill.data?.amount ?? 0)}</td></tr>
                  <tr className="border-t"><td className="py-2 text-muted-foreground">Paid to date</td><td className="py-2 text-right text-muted-foreground">{formatCurrency(totalPaid)}</td></tr>
                  <tr className="border-t font-semibold"><td className="py-2">Outstanding</td><td className="py-2 text-right">{formatCurrency(outstanding)}</td></tr>
                </tbody>
              </table>
              <div className="mt-3">{bill.data && <BillStatusBadge status={bill.data.status} />}</div>
            ...
            <Card>
              <CardHeader><CardTitle className="text-sm">Payments on this bill</CardTitle></CardHeader>
              <CardContent>
                  ...
                  <ul className="divide-y">
                    {(payments.data ?? []).map((p) => (
                      <li key={p.id} className="flex items-center justify-between py-2 text-sm">
                        <div>
                          <Link to="/payments/$paymentId/receipt" params={{ paymentId: p.id }} className="font-mono text-xs text-primary hover:underline">
                            {p.receipt_number}
                          </Link>
                          <p className="text-xs text-muted-foreground">{formatDate(p.paid_at)} · {p.method.toUpperCase()}{p.reference ? ` · ${p.reference}` : ""}</p>
                        </div>
            ...
            {isFullyPaid ? (
              ...
              <>
                ...
                <div className="space-y-1">
                  ...
                  <Input value={form.reference} onChange={(e) => setForm({ ...form, reference: e.target.value })} placeholder="Transaction ID, slip no., etc." />
                </div>
                <Button
                  className="w-full"
                  onClick={() => pay.mutate()}
