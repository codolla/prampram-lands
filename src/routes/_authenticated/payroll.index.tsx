...
function PayrollIndex() {
  ...
  const { data: mySlips } = useQuery({
    queryKey: ["my_payslips", user?.id],
    enabled: !!user?.id,
    queryFn: async () => {
      const { data, error } = await supabase
        .from("payslips")
        .select("*, payroll_runs(period_year, period_month, status)")
        .eq("user_id", user!.id)
    ...
    <AppShell title="Payroll">
      <div className="space-y-6">
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead className="text-left text-muted-foreground">
                      <tr><th className="py-2">Period</th><th>Status</th><th>Gross</th><th>Deductions</th><th>Net</th><th></th></tr>
                    </thead>
                    <tbody>
                      {runs?.map((r) => (
                        <tr key={r.id} className="border-t">
                          <td className="py-2">{MONTHS[r.period_month - 1]} {r.period_year}</td>
                          <td><Badge variant={r.status === "paid" ? "default" : r.status === "finalized" ? "secondary" : "outline"}>{r.status}</Badge></td>
        ...
        <Card>
          ...
          <CardContent>
            {(!mySlips || mySlips.length === 0) ? (
              <p className="text-sm text-muted-foreground">No payslips yet.</p>
            ) : (
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead className="text-left text-muted-foreground">
                    <tr><th className="py-2">Period</th><th>Gross</th><th>Deductions</th><th>Net</th><th>Paid</th><th></th></tr>
                  </thead>
                  <tbody>
                    {mySlips.map((s) => {
                      const r = (s as { payroll_runs: { period_year: number; period_month: number } | null }).payroll_runs;
                      return (
                        <tr key={s.id} className="border-t">
                          <td className="py-2">{r ? `${MONTHS[r.period_month - 1]} ${r.period_year}` : "—"}</td>
                          ...
                          <td>{s.paid ? formatDate(s.paid_at) : <Badge variant="outline">Pending</Badge>}</td>
                          <td className="text-right">
