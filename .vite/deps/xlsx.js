...
function datenum_local(v, date1904) {
  ...
  return (
    (epoch - (SSFdnthresh + (v.getTimezoneOffset() - SSFbasedate.getTimezoneOffset()) * 6e4)) /
    (24 * 60 * 60 * 1e3)
  );
}
...
var dnthresh =
  basedate.getTime() + (refdate.getTimezoneOffset() - basedate.getTimezoneOffset()) * 6e4;
...
var _XLSIcv = rgbify([
  /* Color Constants */
  0, 16777215, 16711680, 65280, 255, 16776960, 16711935, 65535 /* Overridable Defaults */, 0,
  16777215, 16711680, 65280, 255, 16776960, 16711935, 65535, 8388608, 32768, 128, 8421376, 8388736,
  32896, 12632256, 8421504, 10066431, 10040166, 16777164, 13434879, 6684774, 16744576, 26316,
  13421823, 128, 16711935, 16776960, 65535, 8388736, 8388608, 32896, 255, 52479, 13434879, 13434828,
  16777113, 10079487, 16751052, 13408767, 16764057, 3368703, 3394764, 10079232, 16763904, 16750848,
  16737792, 6710937, 9868950, 13158, 3381606, 13056, 3355392, 10040064, 10040166, 3355545,
  3355443 /* Other entries to appease BIFF8/12 */, 16777215 /* 0x40 icvForeground ?? */,
  0 /* 0x41 icvBackground ?? */, 0 /* 0x42 icvFrame ?? */, 0 /* 0x43 icv3D ?? */,
  0 /* 0x44 icv3DText ?? */, 0 /* 0x45 icv3DHilite ?? */, 0 /* 0x46 icv3DShadow ?? */,
  0 /* 0x47 icvHilite ?? */, 0 /* 0x48 icvCtlText ?? */, 0 /* 0x49 icvCtlScrl ?? */,
  0 /* 0x4A icvCtlInv ?? */, 0 /* 0x4B icvCtlBody ?? */, 0 /* 0x4C icvCtlFrame ?? */,
  0 /* 0x4D icvCtlFore ?? */, 0 /* 0x4E icvCtlBack ?? */, 0 /* 0x4F icvCtlNeutral */,
  0 /* 0x50 icvInfoBk ?? */, 0,
  /* 0x51 icvInfoText ?? */
]);
  ...
  while ((Rn = xlmlregex.exec(str)))
    switch ((Rn[3] = (raw_Rn3 = Rn[3]).toLowerCase())) {
      ...
      default:
        ...
        if (
          state[state.length - 1][0] /*'CustomDocumentProperties'*/ === "customdocumentproperties"
        ) {
          if (Rn[0].slice(-2) === "/>") break;
          else if (Rn[1] === "/")
        ...
        case 2:
          {
            if (out["!type"] == "chart") {
              if (
                options.dense ? (out[val.r] || [])[val.c] : out[encode_cell({ c: val.c, r: val.r })]
              )
                ++val.c;
            }
